...
import com.js.work.mapper.EmpMapper;
...
//			가장 급여가 높은사람 empno 찾아 내기
			...
			for(EmpVO vo : list) {
				if(sal <= vo.getSal()) { //급여 비교
					sal = vo.getSal();
					empNoList.add(vo.getEmpno()); //급여 많이 받는 사원 list 추가
				}
			}
...
//		필요에 따라 고객 요구사항이 들어오면 여기서 로직을 구현하면 된다.
		int pageSize = 10;
